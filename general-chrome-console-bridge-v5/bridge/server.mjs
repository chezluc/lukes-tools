...
function healthPayload() {
  ...
  return {
    ...
    hint: online.length > 0
      ...
      : "No extension worker polling. Make sure the browser is running with the v5 extension enabled; its bridge.html worker tab opens automatically.",
  };
}
    ...
    try {
      ...
      if (onlineWorkers().length === 0) {
        return apiError(response, 503, "no_extension_worker",
          "No extension worker is polling this queue. Start the target browser (the v5 extension auto-opens its worker tab). If the browser is running, check chrome://extensions that 'Chrome Console Bridge v5' is enabled.");
      }
...
server.listen(PORT, HOST, () => {
  console.log(`Chrome Console Bridge v5 [${INSTANCE}] listening on http://${HOST}:${PORT}`);
});
